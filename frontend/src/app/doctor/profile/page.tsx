...
interface Profile {
    ...
    gender?: boolean;
    image?: string;
    specialtyData?: { id: number; name: string };
                ...
                <div className="bg-white rounded-xl border p-6">
                    <div className="flex flex-col lg:flex-row gap-6">
                        ...
                        <div className="flex flex-col items-center lg:w-48 flex-shrink-0">
                            ...
                            <h2 className="text-lg font-bold text-gray-900 text-center">Dr. {profile?.firstName} {profile?.lastName}</h2>
                            <p className="text-gray-500 text-sm mb-2">{profile?.email}</p>
                            <div className="flex flex-wrap gap-1 justify-center">
                                {profile?.positionData && <span className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded-full text-xs">{profile.positionData.valueVi}</span>}
                                {profile?.specialtyData && <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-xs">{profile.specialtyData.name}</span>}
                            </div>
                        </div>
                        ...
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-xs font-medium text-gray-500 mb-1">Học vị / Chức danh</label>
                                <p className="text-gray-900 font-medium">{profile?.positionData?.valueVi || '-'}</p>
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-500 mb-1">Chuyên khoa</label>
                                <p className="text-gray-900 font-medium">{profile?.specialtyData?.name || '-'}</p>
                            </div>
                        </div>
                    </div>
                    <p className="text-xs text-gray-400 mt-4 pt-4 border-t">* Thông tin chuyên môn chỉ có thể được thay đổi bởi quản trị viên</p>
                </div>
            </div>
        </div>
    );
